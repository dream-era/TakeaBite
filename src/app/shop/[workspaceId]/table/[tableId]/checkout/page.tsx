  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedMethod === 'online' && result.razorpay) {
        ...
        clearCart(workspaceId);
        const nextUrl = `/shop/${workspaceId}/table/${tableId}/order-confirmation?id=${result.orderId || result.id}&token=${result.orderToken}`;
        router.push(nextUrl);
      }
    } catch (error: unknown) {
      ...
      <main className="flex-grow pb-32 px-container-padding py-6 space-y-8">
        ...
        <div className="bg-surface-container-low rounded-xl p-4 flex items-center justify-between border border-surface-container-high">
          <div className="flex items-center gap-3">
            ...
            <span className="font-label-md text-on-surface">Dining at Table</span>
          </div>
          <span className="font-headline-md text-primary bg-primary/10 px-3 py-1 rounded-md">{tableLabel}</span>
        </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-50 bg-surface shadow-[0px_-4px_16px_rgba(0,0,0,0.08)] backdrop-blur-md pb-safe">
        <div className="max-w-md mx-auto p-4 flex items-center gap-4">
          ...
          <button 
            ...
            className={`flex-1 flex justify-center items-center h-[48px] bg-primary text-on-primary font-label-lg rounded-xl shadow-lg transition-all active:scale-[0.98] ${isSubmitting ? 'opacity-80 cursor-wait pointer-events-none' : 'hover:opacity-90'}`}
          >
            {isSubmitting ? (
