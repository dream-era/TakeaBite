...
export default function CookDashboardPage() {
  ...
  const relevantOrders = orders
    ...
    .filter(order => order.items.some(i => i.itemType === 'food'))
    .filter(order => order.statusByFood !== 'COMPLETED')
    .sort((a, b) => b.createdAt - a.createdAt); // Newest first default
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col items-center">
              <h1 className="font-bold text-[17px] leading-tight tracking-wide">Cook Dashboard</h1>
              <p className="text-[#999999] text-[11px]">Manage & prepare food orders</p>
            </div>
          </div>
          ...
          <div className="relative p-2 -mr-2">
            ...
            <div className="absolute top-1 right-1 w-4 h-4 bg-[#D32F2F] rounded-full flex items-center justify-center text-[10px] font-bold border-2 border-[#1a1a1a]">
              {relevantOrders.filter(o => o.statusByFood === 'NEW').length}
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="px-4 pb-6 space-y-4">
        {relevantOrders.length === 0 ? (
          ...
          relevantOrders.map(order => {
            const foodItems = order.items.filter(i => i.itemType === 'food');
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase flex items-center gap-1.5 border border-transparent ${getStatusColor(order.statusByFood)}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${getStatusDot(order.statusByFood)}`} />
                        {order.statusByFood}
                      </div>
                  ...
                  <div className="mt-2">
                    {order.statusByFood === 'NEW' && (
                      <button 
                        onClick={() => updateOrderStatus(order.id, 'food', 'PREPARING')}
                    ...
                    {order.statusByFood === 'PREPARING' && (
                      <button 
                        onClick={() => updateOrderStatus(order.id, 'food', 'READY')}
                    ...
                    {order.statusByFood === 'READY' && (
                      <button 
                        onClick={() => updateOrderStatus(order.id, 'food', 'COMPLETED')}
