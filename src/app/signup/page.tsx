...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // DEV BYPASS: Use Admin API to create user & auto-confirm email to bypass rate limits
      const { adminSignUpDev } = await import("@/actions/auth");
      const res = await adminSignUpDev(email, password, fullName);
      
      if (!res.success) {
        throw new Error(res.error);
      }

      // Now log them in on the client side to establish the session
      ...
      // Temporary local storage for onboarding flow if needed
      if (businessName) localStorage.setItem('serveflow_business_name', businessName);
      
      router.push("/onboarding");
    } catch (err: unknown) {
      setErrorMsg(err instanceof Error ? err.message : "An error occurred during signup.");
    } finally {
          ...
          <div className="relative">
            ...
            <div className="relative grid overflow-hidden rounded-3xl border border-white/50 bg-white/60 shadow-2xl backdrop-blur-xl md:grid-cols-2">
              ...
              <div className="flex flex-col justify-center px-8 py-10 sm:px-12 md:py-14">
                <h2 className="text-center text-2xl font-bold text-neutral-900 sm:text-[1.65rem]">
                  Create Your Account
                </h2>
                ...
                <div className="my-5 flex items-center justify-center text-center">
                  <span className="text-xs text-neutral-400">or use your business details for registration</span>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {/* Business Name */}
                  <div className="relative">
                    <Briefcase className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-neutral-400" />
                    <input
                      type="text"
