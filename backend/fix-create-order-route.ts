...
// Validation schema — includes customerName + customerPhone
...
const CreateOrderSchema = z.object({
  ...
  // Customer details — optional but saved when provided
...
function buildOrderItems(orderId: string, items: CartItem[]): OrderItemInsert[] {
  return items.map((item) => ({
    ...
    price: item.price,
    // CRITICAL: station must be set correctly for kitchen routing
    // food → chef dashboard
    // juice → juice maker dashboard
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Rate limit check — wrap in try/catch so Redis failure
    // doesn't block orders (Redis is optional protection)
    try {
      ...
      // Redis unavailable — log but continue
    ...
    // Verify server-side total — prevents price manipulation
    const serverTotal = calculateTotal(items as CartItem[])
    ...
    // Fetch restaurant + menu items for verification
    ...
    // Use DB prices (authoritative) not client prices
    const dbPriceMap = new Map(menuItems.map((m) => [m.id, m.price]))
    // Also use DB station values (more reliable than client)
    const dbStationMap = new Map(menuItems.map((m) => [m.id, m.station as Station]))

    const verifiedItems: CartItem[] = items.map((item) => ({
      ...item,
      price: dbPriceMap.get(item.menuItemId) ?? item.price,
      // FIX: use DB station value for reliable kitchen routing
      station: dbStationMap.get(item.menuItemId) ?? item.station,
    }))

    const verifiedTotal = calculateTotal(verifiedItems)
    ...
    if (paymentMethod === 'cash') {
      ...
      if (orderError || !order) {
        console.error('[ServeFlow] Cash order insert failed:', orderError)
